...
- A trusted controller heartbeat with runner-release state other than `current` automatically enters the same Worker-owned hosted hold with reason `runner-upgrade`; it is not a weaker side flag. That machine-created hold can auto-release only after the current active enrollment session reports the exact previously qualified candidate selected, acquisition disabled, zero unassigned released listeners, and complete current-image compatibility. The qualified tuple survives re-enrollment, while any heartbeat from the superseded session remains invalid. An operator-created hold never auto-releases. Pending, rejected, interrupted, or incompatible candidates remain hosted and continue the bounded outbound observation/retry path without manual intervention.
- An active `operator` hold has strict precedence over runner-upgrade state. While `hosted_hold_reason_code == "operator"`, a non-current runner-release heartbeat persists its validated observation and qualified tuple and revokes/drains acquisition authority, but cannot replace the hold reason, obtain `stage-permitted`, stage/select a candidate, or auto-release. Authenticated operator release leaves routing hosted and starts the normal recovery epoch; if the persisted runner state remains non-current, only a fresh later heartbeat may create a new `runner-upgrade` hold and begin the automatic directive sequence.
...
Public enrollment/health endpoints are `POST /v1/enrollment/challenge`, `POST /v1/enrollment/complete`, and `POST /v1/heartbeat`. Outbound controller authority endpoints are `POST /v1/acquisition/permit`, `POST /v1/acquisition/close`, `POST /v1/acquisition/legacy-lease`, and read-only `POST /v1/runner-maintenance/status`; they use the enrolled fleet key/session and an independent strict control sequence. The maintenance endpoint cannot mutate routing or acquisition; it returns the one currently permitted local replacement/recovery phase. Disabled-by-default administrative endpoints are `POST /v1/admin/recovery`, `POST /v1/admin/legacy-recovery`, `POST /v1/admin/queue-recovery`, `POST /v1/admin/hosted-hold`, and `POST /v1/admin/status`; they use the separate administrative HMAC key, the same timestamp/nonce anti-replay rules, and generic rejection responses. Requests use `Content-Type: application/json`, `X-Portable-GHAR-Key-Id`, `X-Portable-GHAR-Timestamp: <unix-seconds>`, and `X-Portable-GHAR-Signature: v1=<base64url-mac>`. Permit/lease/maintenance responses are MACed over a domain-separated response frame that excludes the `responseMac` field itself: the signed bytes are `portable-ghar-response-v1\n`, then `<protocol>\n`, then the response object serialized with its interface-declared field order, no insignificant whitespace, and the `responseMac` field omitted entirely. The verifier reconstructs the identical frame with `responseMac` removed and compares in constant time. Signed request bytes are:
...
```typescript
export interface RunnerReleaseStatusV1 {
  state: "current" | "upgrade-required" |
    "candidate-qualified" | "candidate-rejected";
...
export interface HeartbeatV1 {
  ...
  runnerRelease: RunnerReleaseStatusV1;
  fatalReasonCode?: "compatibility_failed" | "host_profile_failed" |
    "legacy_probe_failed" | "runner_release_incompatible" |
    "state_ambiguous";
}
...
export interface RunnerMaintenanceStatusRequestV1 {
  protocol: "portable-ghar.runner-maintenance.status.v1";
  fleetId: string; epoch: number; sessionId: string; controlSequence: number;
  selectedManifestDigest: string;
  candidateManifestDigest: string | null;
}

export interface RunnerMaintenanceDirectiveV1 {
  protocol: "portable-ghar.runner-maintenance.directive.v1";
  epoch: number; sessionId: string;
  requestControlSequence: number;
  requestedSelectedManifestDigest: string;
  requestedCandidateManifestDigest: string | null;
  transitionEpoch: number; permitGeneration: number;
  phase: "wait-hosted" | "stage-permitted" | "replace-permitted" |
    "canary-permitted" | "enable-permitted" | "complete";
  qualifiedVersion: string | null;
  qualifiedManifestDigest: string | null;
  qualifiedImageDigest: string | null;
...
export interface AdministrativeStatus {
  ...
  hostedHold: boolean; hostedHoldReason: "operator" | "runner-upgrade" | null;
  runnerReleaseState: RunnerReleaseStatusV1["state"] | "unobserved";
  runnerSelectedManifestDigest: string | null;
  runnerCandidateManifestDigest: string | null;
...
export interface FleetCoordinator {
  ...
  runnerMaintenanceStatus(input: { request: RunnerMaintenanceStatusRequestV1; receivedAtMs: number; config: FleetConfig }): Promise<RunnerMaintenanceDirectiveV1>;
...
Runner versions match `^v[0-9]+\.[0-9]+\.[0-9]+$` and compare as three
unsigned integer components, never lexically. Runner manifest and image digests
are lowercase `sha256:` plus exactly 64 hexadecimal characters.
`observationSequence` is strictly increasing within one enrollment session.
For `current`, `observedVersion == selectedVersion` and all candidate fields
are null. For `upgrade-required`, `observedVersion > selectedVersion` and all
candidate fields are null. For `candidate-qualified`, every candidate field is
non-null, `candidateVersion == observedVersion > selectedVersion`, and the
Worker persists that exact candidate tuple before accepting later selection
proof. For `candidate-rejected`, `candidateVersion == observedVersion >
selectedVersion`; unavailable manifest/image digests may remain null and cannot
authorize selection. A later `current` after a runner-upgrade hold can match
only the remembered qualified version/manifest/image tuple; regression,
skipped observation sequence, tuple substitution, or a `current` claim without
that match remains hosted.

The maintenance directive is read-only and short-lived. `wait-hosted` is the
default on any incomplete, stale, or ambiguous predicate, including an active
`operator` hold. Every non-wait phase requires the requested selected manifest
to equal the latest authenticated selected manifest. `stage-permitted` further
requires a `runner-upgrade` hold, a non-null requested candidate manifest
different from the selected manifest, fully drained permit generation, every
repository confirmed hosted, latest queue risk cleared, and a current
heartbeat with zero assigned jobs and zero unassigned released listeners. It
authorizes local acquisition disable plus bounded staging and qualification of
only that requested candidate; it does not authorize selection.
`replace-permitted` additionally requires the requested candidate manifest to
equal the exact candidate-qualified tuple persisted from a later heartbeat and
local acquisition already disabled. `canary-permitted` requires that tuple
selected and reported `current`, zero unassigned released listeners, current
compatibility success, and automatic hold release into its new transition
epoch. `enable-permitted` additionally requires that epoch's exact canary
success; `complete` requires the later enabled/full-capacity heartbeat and
self-hosted read-back. Every response is bound to the current enrollment
epoch/session, exact request control sequence and requested selected/candidate
manifest digests, Worker transition epoch, permit generation, configuration
revision, qualified tuple, expected canary/enabled policy digests, and
server-owned expiry. The local upgrader fetches and revalidates a fresh
directive immediately before each local phase transition; a cached, expired,
wrong-request, wrong-phase, wrong-tuple, wrong-policy, or superseded response
cannot authorize staging, replacement, or acquisition.
...
fleet_state(
  ...
  runner_release_state, runner_observation_sequence,
  runner_observed_version, runner_selected_version,
  runner_selected_manifest_digest, runner_selected_image_digest,
  runner_candidate_version, runner_candidate_manifest_digest,
  runner_candidate_image_digest,
  qualified_runner_version, qualified_runner_manifest_digest,
  qualified_runner_image_digest,
...
- An accepted `upgrade-required`, `candidate-qualified`, or
  `candidate-rejected` runner-release status enters that same hold with reason
  `runner-upgrade`, revokes/drains permits, persists the exact release tuple,
  and routes hosted before local replacement, unless an `operator` hold is
  already active. In that case it persists the validated release tuple and
  revokes/drains permits without changing the hold reason or issuing a
  maintenance phase. Repeated/reordered observations are idempotent or
  rejected; they cannot create a second hold generation.
- Only a `runner-upgrade` hold may auto-release. Release requires the current
  active enrollment session to report `current` with the exact remembered
  qualified version/manifest/image tuple, acquisition `disabled`, zero
  unassigned released listeners, current compatibility success, every
  repository confirmed hosted, and the latest queue-risk generation cleared.
  The qualified tuple persists across controller re-enrollment; old-session
  heartbeats remain rejected by the normal epoch/session check.
  It starts a new recovery epoch and still requires the normal canary plus
  enabled-heartbeat gate. A hold whose reason is `operator` requires the
  authenticated administrative release and never follows this automatic path.
  If its persisted runner-release state is still non-current afterward, only a
  fresh later heartbeat may create a new `runner-upgrade` hold; the
  administrative release itself never converts the hold reason or issues
  `stage-permitted`.
- A rejected, missing, stale, incompatible, or interrupted candidate leaves the
  `runner-upgrade` hold active. Later outbound observations may qualify a new
  monotonic candidate without operator action, but no candidate state issues
...
- [ ] Implement the locked timestamped request and domain-separated response framing, strict raw base64url decoding, 32-byte keys/MACs, client-generated random 32-byte request nonce, server-generated random 32-byte single-use challenge bound to that nonce digest, `EnrollmentCompleteRequestV1` carrying the same nonce plus challenge and a client-generated random 16-byte session ID, server epoch response, independent heartbeat/control sequences starting at `1`, and one bounded re-enrollment attempt after authenticated `409`. Verify HMAC before claiming the initial request nonce; completion atomically verifies the bound pair and consumes the challenge exactly once. Publish only validated runner-release tuples from the controller-owned observer/upgrade journal; no job-controlled version, digest, or state reaches the heartbeat. The remote provider requests a fresh operation-bound permit only from inside the controller critical section, validates the exact MAC/bindings/server expiry, retains it only in memory across that one external call, and sends an idempotent close; the legacy wrapper renews its process lease before the fail-closed termination deadline. The maintenance provider is read-only, consumes the same strict control sequence, verifies every signed phase binding and expiry, and never turns a directive into routing authority. Never persist a reusable permit/lease bearer or server epoch locally, or log bodies, timestamps, nonces, headers, keys, challenges, sessions, permit/lease IDs, or request/response MACs.
...
- [ ] Add runner-release state-machine and directive tests: every accepted non-`current` status enters one idempotent `runner-upgrade` hold and permit drain when no operator hold exists; candidate rejection/pending/interruption stays hosted; a later monotonic qualified tuple survives eviction; tuple substitution, observation rollback, current-without-qualification, acquisition non-disabled, live assigned jobs/listeners, open queue risk, or incomplete hosted read-back cannot advance the corresponding phase or auto-release; the exact qualified selection proof auto-releases only a `runner-upgrade` hold into a new recovery epoch; and an `operator` hold never auto-releases or changes reason. Cover the overlap explicitly: operator hold + `upgrade-required`/qualified/current heartbeats persists release evidence and drain but returns only `wait-hosted`; authenticated operator release still does not convert the reason; only the next fresh non-current heartbeat enters a new `runner-upgrade` hold and may reach `stage-permitted`. Prove the directive mapping `wait-hosted -> stage-permitted -> replace-permitted -> canary-permitted -> enable-permitted -> complete`, default-to-wait on every incomplete predicate, short server expiry, exact current-session/request/transition/permit/config/tuple/policy bindings, and no state-changing side effect from the status request itself.
...
- [ ] Integrate runner-release maintenance into that same locked state machine. Any accepted non-current status first persists permit revocation/drain and, only when no operator hold exists, a `runner-upgrade` hold; alarm-driven drain completion then creates hosted intent and queue risk. Persist the observation and qualified tuple transactionally across re-enrollment even while an operator hold dominates, but never overwrite that reason or issue a non-wait maintenance directive. Auto-release only the `runner-upgrade` reason after exact current-active-session selection of the remembered tuple, disabled-acquisition, zero-listener, compatibility, hosted-read-back, and queue-clear proof; reject superseded-session claims, create the normal new recovery epoch, and retain hosted routing through canary and acquisition-enabled confirmation. Authenticated operator release remains manual and cannot convert into an upgrade hold; if the persisted release is still non-current, a fresh later heartbeat performs that transition. Candidate rejection or ambiguity never clears either hold.
...
- [ ] A non-current runner-release heartbeat automatically enters the Worker-owned `runner-upgrade` hold only when no operator hold exists; rejected/interrupted candidates remain hosted; only exact qualified-candidate selection with disabled acquisition, zero listeners, compatibility, hosted read-back, and queue clearance auto-releases it. An operator hold is never overwritten or auto-released, yields only `wait-hosted`, and after manual release requires a fresh non-current heartbeat before the upgrade sequence can begin.

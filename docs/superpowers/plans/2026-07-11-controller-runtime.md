...
- [ ] **Step 2: Prove the intended red state.** Run `GOTOOLCHAIN=go1.26.5 go test ./internal/buildinfo -run 'TestPins(ExactValues|FieldTable)$' -count=1`. Expected: FAIL only because `Pins().UpstreamRunner` still returns `v2.335.1` and its prior Linux x64 digest.
...
- [ ] **Step 4: Verify the focused and repository suites.** Re-run the focused command, then `GOTOOLCHAIN=go1.26.5 go test ./...` and `git diff --check`. Expected: PASS; the exact new version/digest have one implementation source, and no task has enabled in-place runner update.
...
- [ ] **Step 6: Build immutable seeds and verify.** Copy verified seeds mode 0555 into `/opt/portable-ghar/seed-cache` on the read-only image; `hydrate-seeds` copies selected content into that runner's private tmpfs before listener release. Absence of a manifest builds an empty seed cache. Run `GOTOOLCHAIN=go1.26.5 go test -race ./internal/hostruntime ./internal/archive ./cmd/portable-ghar-runner-gate ./cmd/portable-ghar-network-adapter && bats tests/shell/fetch-runner.bats tests/shell/action-tool-archive.bats`. Expected: `SourceVerification` PASS, including corrupt digest/license, skipped OS-identity verification, mid-hash mutation, hardlink/mount crossing, tracked archive, immutable seed, relay same-UID replacement/flood/path/mount tests, cross-job mutation tests, version-smoke/tree-lock failure, duplicate-version/update-staging rejection, closed operation ordering, and a compile-time/projection test proving source-only evidence cannot construct deployment eligibility. Linux-only seccomp enforcement, cgroup/tmpfs, namespace, peer-instance, image/tree, process-inventory, listener, whole-container reclamation, and forced-version-bump proofs remain typed `TargetConformance` pending; neither unit tests nor macOS Dockerfile inspection may aggregate them into green. No live runner file sweeper is added. After exact-diff distinct-family review and all locally executable gates pass, commit: `git commit -S -m "feat: create held runner with verified immutable seeds"`.

...
**Tech Stack:** Go language level 1.26.0 with toolchain 1.26.5; `github.com/actions/scaleset v0.4.0`; `modernc.org/sqlite v1.53.0`; `github.com/google/nftables v0.3.0`; Docker Engine CLI; nftables; POSIX shell plus Bats for QTS installation tests; Go unit, contract, integration, conformance, race, and chaos tests.
...
- Unsupported kernels, Docker builds, cgroup enforcement, nftables support, route discovery, or non-root behavior fail closed. Host pressure may reduce capacity and never increase configured ceilings; every effective reduction traverses the acquisition-policy epoch barrier before it takes effect.
...
**Files:** Create `internal/hostruntime/engine.go`, `internal/hostruntime/command.go`, `internal/hostruntime/dockercli.go`, `internal/hostruntime/dockercli_test.go`, `internal/archive/{manifest,verify}.go` and tests, `config/schema/action-tool-archive-manifest.schema.json`, `cmd/portable-ghar-runner-gate/main.go` and tests, `scripts/{fetch-runner,stage-action-tool-archive}.sh`, `tests/shell/action-tool-archive.bats`, `images/runner/Dockerfile`.

**Interfaces:** `Engine.CreateRunner(ctx, RunnerSpec, SecretEnvFD) (Container, error)`; `Start`, `Inspect`, `Wait`, `ExecStdin`, `RunOneShot`, `ListManaged`, `Remove`; `archive.Load(io.Reader) (Manifest, error)` and `archive.Verify(fs.FS, Manifest) (Digest, error)`; `CommandRunner.Run(ctx, argv []string, extraFiles []*os.File, stdin io.Reader) (Result, error)`; gate subcommands `hold`, `release`, `hydrate-seeds`, and `netns-id`. Add compile-time interface assertions against the canonical contract.

- [ ] **Step 1: Write failing argv and gate tests.** Assert runner creation has read-only root, bounded executable work/tmp tmpfs, CPU/memory/PID/scratch limits, cap-drop ALL, no-new-privileges, seccomp, non-root user, no mounts/volumes/devices/socket, explicit public DNS, no restart, opaque labels, and one Docker-default unique netns. Assert wrong, absent, duplicate, and reused readiness tokens never exec the listener.
...
**Interfaces:** `PolicyManifest{Mode, PublicDNS, DynamicDeny, DockerHost, PositiveProbes, NegativeProbes}`; `Compile(PolicyManifest) (Ruleset, Digest, error)`; `Jail.Configure(ctx, Container, PolicyManifest) (Verification, error)`; helper reads canonical JSON on stdin; verifier returns `ProbeReport{PolicyDigest, NetNSID, PositiveOK, NegativeOK}`.

- [ ] **Step 1: Write failing range/compiler tests.** Cover IPv4 unspecified, RFC1918, loopback-off-interface, link-local/metadata, CGNAT, protocol/reserved, documentation/benchmark, multicast, and reserved ranges; IPv6 unspecified, loopback-off-interface, mapped/NAT64, discard, documentation/ORCHID/6to4, ULA, link-local/metadata, multicast, and dynamic host/bridge/management routes. Default mode must deny `::/0`; dual-stack may allow only the remainder.
- [ ] **Step 2: Write failing orchestration tests.** Require exact order `runner held -> helper NET_ADMIN-only -> helper exit/gone -> capless verifier -> matching unique netns and policy digest -> readiness release`. Kill, timeout, contradictory report, unexpected route, policy corruption, helper residue, verifier capability, or token failure must remove the runner and never release.
- [ ] **Step 3: Implement atomic nftables policy.** Use nftables v0.3.0 in a one-shot static helper, default-drop output, allow loopback only on `lo` and established traffic, reject denied sets, allow the selected public family remainder, emit only digest/netns/status, and accept trusted controller input only. The verifier uses ordinary DNS/TCP probes and has cap-drop ALL.
- [ ] **Step 4: Verify unit contracts.** Run `GOTOOLCHAIN=go1.26.5 go test -race ./internal/networkjail ./cmd/portable-ghar-network-helper ./cmd/portable-ghar-network-verifier -count=20`. Expected: PASS and the engine fake observes no helper/listener overlap.
...
- [ ] **Step 1: Write failing profile and fence tests.** Reject non-Linux, unsupported architecture/runtime/kernel, missing resource enforcement, nftables failure, incomplete route discovery, unsafe private-file mode, automatic degraded-root selection, malformed state, stale generation, wrong active fleet, reused owner/PID/boot identity, and unlocked mutation. Accept degraded root only with an exact configured profile name and surface it in health. Test multiple same-fleet holder records, stable-lock inode identity across header replacement, blocked exclusive acquisition, crash release, per-holder renewal, stale holder cleanup, and force-disabled observer restart while `legacy` owns the fence.
